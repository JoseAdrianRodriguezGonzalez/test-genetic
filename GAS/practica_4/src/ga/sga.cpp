...
void algoritmogeneticsimple::Elitismo(void) {
  ...
  if (this->tc == BIN) {
    ...
    for (unsigned int i = 0; i < this->tamaño_cromosoma; i++) {
      this->poblacion_nueva[0].valores_reales[i] =
          this->poblacion[this->id_mejor_objetivo].valores_reales[i];
    }
  }
}
...
void algoritmogeneticsimple::fit(
  ...
  for (unsigned int gen = 1; gen <= this->max_gen; gen++) {
    ...
    if (method.cruza == "unico" && this->tc == BIN)
      this->CruzamientoPuntoUnico(this->probabilidad_cruza);
    if (method.cruza == "doble" && this->tc == BIN)
      this->CruzamientoDoblePunto(this->probabilidad_cruza);
    if (method.cruza == "uniforme" && this->tc == BIN)
      this->CruzamientoUniforme(this->probabilidad_cruza);
    if (this->tc == REAL) {
      this->CruzaReal(this->probabilidad_cruza);
      // this->ImprimirPoblacion();
      this->mutaReal(this->probabilidad_mutacion, 0.1);
    }
    if (this->tc == BIN)
  ...
  cout << "Mejor valor= " << this->ObtenerMejorObjetivo() << "\n";
}
